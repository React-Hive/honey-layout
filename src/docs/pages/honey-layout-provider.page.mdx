import { HoneyLayoutProviderExample, BasicHoneyOverlayBasixExample } from './examples'
...
The `HoneyOverlay` component is a flexible tool for creating overlays such as modals, dialogs, and popups.
It simplifies managing overlay state and supports keyboard interactions like closing with the "Escape" key.

### Key Features

1. **Declarative Overlay State Management:** Activate or deactivate the overlay using the `isActive` prop.
2. **Keyboard Interaction**: Built-in support for handling keyboard events like closing the overlay with the "Escape" key.
3. **Customizable Content:** Render static or dynamic content, passing the overlay object as needed.

### Usage

<BasicHoneyOverlayBasixExample/>
  ...
  return (
    <div>
      ...
      >
        ...
        <button onClick={handleClose}>Close Overlay</button>
      </HoneyOverlay>
    </div>
  );
};
