...
export const LazyContentExample = () => {
  ...
  return (
    <HoneyBox $padding="16px" $border="1px solid #cccccc" $borderRadius="4px">
      ...
      <HoneyBox>
        <HoneyLazyContent isMount={isShow} unmountDelay={1000}>
...
* **isMount**: Determines whether the content should be mounted or unmounted.
* **unmountDelay**: The delay in milliseconds before unmounting the content when `isMount` is set to `false`.
* **isAlwaysMounted**: Determines whether the content should always remain mounted, regardless of the value of `isMount`.
If `true`, the content will never be unmounted. Defaults to `false`.
* **isKeepAfterMount**: Determines whether the content should remain mounted after the mount.
...
import { HoneyBox, HoneyLazyContent, resolveSpacing } from '@react-hive/honey-layout';
